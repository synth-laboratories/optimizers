...
impl MapoConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for seed in &self.taskset.selection_seeds {
            if self.taskset.heldout_seeds.contains(seed) {
                return Err(OptimizerError::Config(format!(
                    "taskset.selection_seeds must not overlap heldout_seeds; seed {seed} is heldout"
                )));
            }
        }
