...
credential launch resolution, run-local Codex homes, Codex app-server JSON-RPC,
and Codex usage normalization.
...
- `app_server`: starts `codex app-server`, sends JSON-RPC requests, waits for
  turn lifecycle events, and drains stderr for actionable failures.
