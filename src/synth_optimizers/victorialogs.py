...
def _slot_victorialogs_port(slot: str) -> int | None:
    for root in [Path.cwd(), *Path.cwd().parents]:
        ...
        if candidate.is_file():
            match = re.search(r"^\s*victorialogs\s*=\s*(\d+)\s*$", candidate.read_text(), re.MULTILINE)
            if match:
...
def _vl_slot() -> str:
    return _safe_slot(os.environ.get("STACK_VL_SLOT") or os.environ.get("SYNTH_OPTIMIZERS_VL_SLOT") or DEFAULT_SLOT)
