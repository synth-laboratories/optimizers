#!/usr/bin/env python3
"""Five-update, verifier-backed Harbor TBLite CISPO canary on Tinker."""
...
from synth_optimizers.providers.protocols import SampleRequest, TrainingStepRequest  # noqa: E402
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:  # noqa: N802
                try:
                    ...
                    sampled = owner.adapter.sample(owner.session, SampleRequest(
                        ...
                        temperature=0.8, seed=int(time.time_ns() % 2_000_000_000),
                    ))
...
def register_policy(base: str, gateway_port: int, checkpoint: str, update: int, member: int) -> str:
    cid = f"tblite_cispo_u{update:02d}_m{member:02d}_policy"
    request(base, "POST", "/policy-configs", {
        "config_id": cid, "harness": "mini_swe", "config": {
            "model": "openai/gpt-oss-20b", "model_path": checkpoint,
            "base_url": f"http://host.docker.internal:{gateway_port}/v1/u{update}/m{member}",
            "api_key_env": "TINKER_API_KEY", "inference_transport": "chat_completions",
            "max_steps": 50, "max_tokens": 4096, "temperature": 0.8,
            "sampling_seed": update * 1000 + member, "command_timeout_seconds": 300,
...
def rollout(base: str, cid: str, task: str, update: int, member: int) -> dict:
    rid = f"tblite_cispo_u{update:02d}_m{member:02d}_{int(time.time())}"
...
def main() -> None:
    ...
    gateway.start()
    summary = {"schema_version": "harbor.tblite.cispo.v1", "steps": [], "model": session.model_id}
    for update in range(1, args.steps + 1):
        task = TASKS[(update - 1) % len(TASKS)]
        configs = [register_policy(base, args.gateway_port, checkpoint.provider_reference, update, member) for member in range(args.cardinality)]
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.cardinality) as pool:
            futures = [pool.submit(rollout, base, configs[member], task, update, member) for member in range(args.cardinality)]
