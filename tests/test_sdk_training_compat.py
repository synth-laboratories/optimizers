...
from synth_optimizers.hosted import HostedOptimizerClient, OptimizerAlgorithmSlug, submit_cispo, submit_sft
...
def test_submit_cispo_helper_still_requires_preflight() -> None:
    class FakeClient:
        def submit_cispo(self, config, **kwargs):
            return config, kwargs

    config, kwargs = submit_cispo({"model": {"id": "gpt-oss-20b"}}, client=FakeClient())
    assert config["model"]["id"] == "gpt-oss-20b"
