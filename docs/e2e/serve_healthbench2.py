"""Serve HealthBench2's CISPO surface over real HTTP, with a real sampler.

Wired the way the image's own tests wire it -- the deterministic rubric judge,
so nothing is bought -- except that the transport posts to whatever origin the
executor bound instead of synthesizing an answer.
"""
...
IMAGE = Path("/Users/joshuapurtell/GitHub/evals/containers/images/healthbench2")
sys.path.insert(0, str(WORKTREE / "src"))
sys.path.insert(0, str(IMAGE))

import uvicorn  # noqa: E402

from healthbench_chat import cispo  # noqa: E402
from healthbench_chat.targets import HEALTHBENCH_CHAT  # noqa: E402
...
class HttpSampler:
    ...
    def _probe_answer(self, body: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        answer = "Seek urgent in-person care and call emergency services now."
