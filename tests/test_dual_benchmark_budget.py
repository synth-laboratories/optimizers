...
def test_concurrent_reservations_cannot_cross_cap(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(budget, 'LEDGER_ROOT', tmp_path)
    def claim(_):
        try:
            return budget.reserve('test', 30)
        except RuntimeError:
            return None
    ...
    with budget.connect() as db:
        assert db.execute('SELECT SUM(reserved) FROM charges').fetchone()[0] == 90
    budget.settle(keys[0], 1, {'test':True})
    assert budget.reserve('test', 30)
